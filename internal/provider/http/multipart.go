...
// tuple of parts produced by the runtime. File parts using `path` are
// resolved relative to the .tales file owning the step. The boundary is
// generated by mime/multipart; callers must use the returned Content-Type
// header verbatim so the receiver can find that boundary.
func encodeMultipartBody(input provider.Input, value cty.Value) ([]byte, string, error) {
	...
	if !value.Type().IsTupleType() && !value.Type().IsListType() {
		return nil, "", fmt.Errorf("request.body.multipart must be a list")
	}
