...
Suite teardown (e2e/pass/_suite.tales)
  ✔ purge_test_users                              9ms
...
The `Suite teardown` block is printed after the last scenario, and only when the suite declares a top-level `teardown { ... }`. Suites without one produce exactly the same output as before.

Every failed scenario also prints a replay command. The path it shows is the one the run was launched with, so it can be pasted as-is: a directory run also loads sibling `_config.tales` and `keywords/` files that the failing scenario's own file would not pull in.
