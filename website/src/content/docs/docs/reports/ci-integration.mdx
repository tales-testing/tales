...
description: Drop-in recipes for running Tales on GitHub Actions, GitLab CI, and CircleCI — with JUnit, JSONL, and visual HTML artifacts.
---
...
1. **Install the binary** (one of: release archive, `go install`, source build).
		      ...
		      - name: Install Tales
		        run: |
		          curl -fsSL -o tales.tar.gz \
		            https://github.com/hyperxlab/tales/releases/latest/download/tales_0.1.0_linux_x86_64.tar.gz
	...
	<TabItem label="GitLab CI">
		...
		integration:
		  ...
		  before_script:
		    ...
		    - curl -fsSL -o tales.tar.gz https://github.com/hyperxlab/tales/releases/latest/download/tales_0.1.0_linux_x86_64.tar.gz
		    - tar -xzf tales.tar.gz
		    - install -m 0755 tales /usr/local/bin/tales
		  script:
