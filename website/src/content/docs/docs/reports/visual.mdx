...
description: The Tales visual HTML report — single-file, offline, action-by-action playback with screenshots.
---
...
<Aside type="caution" title="Secure inputs are masked at one boundary">
	`input_text` actions marked `secure = true` are masked to `"***"` inside the mobile provider, before the value enters the `ActionResult`. All downstream consumers (HTML data island, JSONL `action` events, console summary) read the already-masked value. The driver still receives the plaintext (otherwise typing wouldn't work), but `hierarchy.json` does not echo password fields by default — verify for any new field you add.
</Aside>
...
- **JSONL** ([`--report-jsonl`](/docs/reports/jsonl/)) — emits one `"type":"action"` event per action after the step event when `step.Actions` is populated. Empty action slices produce byte-identical JSONL to the pre-visual-report format.
- **JUnit** ([`--report-junit`](/docs/reports/junit/)) — unchanged. Action-level data is intentionally not flattened into JUnit XML.
