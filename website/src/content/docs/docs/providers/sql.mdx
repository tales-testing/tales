...
description: The Tales SQL provider — connection configuration, exec / query steps, DSN masking, and limitations.
---

import { Aside, Tabs, TabItem, Card, CardGrid } from '@astrojs/starlight/components';

The `sql` provider is a pragmatic helper for preparing and tearing down application state that is not exposed through the public HTTP / mobile API. It is **not** a replacement for those providers — use it to set up the preconditions, then verify the business behaviour through the same surface your users see.
...
<CardGrid>
	<Card title="Use it for" icon="approve-check">
		...
		- Resetting or seeding a small number of rows for a specific scenario.
		- Inspecting server state inside a `teardown` to make a clean cleanup decision.
	</Card>
	<Card title="Don't use it for" icon="error">
		...
		- Bulk fixtures, migrations, schema management, snapshots — use a real migration tool.
