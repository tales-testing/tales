...
Two things make the difference between a 5-minute job and a 20-minute one:
...
- **Resolve the simulator name up front.** Targets match a simulator by exact name, so a runner image that renames or drops a device fails deep inside the run rather than at startup.
...
jobs:
  ios:
    ...
    steps:
      ...
      - name: Cache the embedded driver build
