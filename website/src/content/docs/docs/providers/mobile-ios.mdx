...
- It is stopped at the end of the scenario, including on failures and panics, via a defer. The recorder sends `SIGINT` only and waits for simctl to flush the MP4 moov atom; a forceful kill would leave the file unplayable.
