...
- **macOS code signing and notarization**, released binaries are not signed. The Homebrew cask runs a `postflight` hook that strips `com.apple.quarantine` automatically, so `brew install --cask tales-testing/tap/tales` users do not see the Gatekeeper prompt. If you extract the tarball directly, run `xattr -d com.apple.quarantine tales` after extraction.
