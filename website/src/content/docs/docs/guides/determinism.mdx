...
2. The scenario name.
3. The **keyword call path** (the chain of calling step names) when the call happens inside a keyword, empty otherwise.
4. The step name.
5. The generator name.
6. The **expression path**, *where* in the step the call appears.

The hash of those inputs seeds a deterministic generator just for that call. The implementation lives in `internal/runtime/seed.go`.
...
### Why the same keyword called twice gets distinct values

When a generator runs **inside a keyword**, the call path (the calling step name, plus the outer call sites for nested keywords) is part of the seed. So calling the same keyword from two different steps produces **different** values, not duplicates:
...
scenario "two distinct signups" {
  step "keyword" "signup_first"  { name = "register_user" }  // email A
  step "keyword" "signup_second" { name = "register_user" }  // email B (different)
}
```

This is what lets a `register_user` keyword be reused across a suite without the second registration colliding with the first (for example, a backend rejecting a duplicate email). A single keyword invocation still produces the same value on every run for the same seed.
