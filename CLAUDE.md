...
For mobile iOS targets, the `driver` block resolves to one of four modes: **embedded** (default — omit `project`, `scheme`, `source_path`), **developer override** (`source_path = "..."`), **legacy** (`project = "..."` + `scheme = "..."`, kept for back-compat), or **external** (`external = true`, only health-checks the URL). See [docs/mobile/ios.md](docs/mobile/ios.md) for details.
