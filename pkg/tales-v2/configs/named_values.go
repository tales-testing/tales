...
func decodeVariableBlock(block *hcl.Block, override bool) (*Variable, hcl.Diagnostics) {
	...
	// reserved attribute and block type names in a "scenario" block, since
	// these won't be usable for child scenarios.
	for _, attr := range scenarioBlockSchema.Attributes {
		if attr.Name == v.Name {
			diags = append(diags, &hcl.Diagnostic{
				Severity: hcl.DiagError,
				Summary:  "Invalid variable name",
				Detail:   fmt.Sprintf("The variable name %q is reserved due to its special meaning inside scenario blocks.", attr.Name),
				Subject:  &block.LabelRanges[0],
			})
		}
	}
	for _, blockS := range scenarioBlockSchema.Blocks {
		if blockS.Type == v.Name {
			diags = append(diags, &hcl.Diagnostic{
				Severity: hcl.DiagError,
				Summary:  "Invalid variable name",
				Detail:   fmt.Sprintf("The variable name %q is reserved due to its special meaning inside scenario blocks.", blockS.Type),
				Subject:  &block.LabelRanges[0],
			})
		}
	}
