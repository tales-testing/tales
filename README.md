...
- **File & exec providers** for artifact-verification flows: HTTP `save { body = "..." }` writes a downloaded body to a per-scenario workspace (`scenario.workdir` / `project.dir`) and exposes `response.download.{path,size_bytes,sha*}`; `step "file"` asserts a file's existence/size/hash/text/JSON; `step "exec"` runs an external verifier directly (no shell), capturing `exit_code` / `stdout` / `stderr` and parsing `stdout_json`. Exec is **disabled by default** — opt in with `--allow-exec`; its `process` sandbox is a hygiene feature, not a security boundary. See [docs/providers/file/](https://taleslabs.org/docs/providers/file/) and [docs/providers/exec/](https://taleslabs.org/docs/providers/exec/).
