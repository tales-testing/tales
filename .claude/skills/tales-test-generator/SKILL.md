...
- Matchers/functions available: `contains`, `matches`, `exists`, `not_exists`, `is_string`, `is_number`, `is_bool`, `is_array`, `is_object`, `one_of`, `can`, `regex_find`, `url_encode`, `jsonencode`, `now_unix`, `now_rfc3339`, `hmac_sha256_hex`, `hmac_sha1_hex`, `totp`, `optional`, `required`, `any`.
...
- `hmac_sha1_hex(secret, message)` returns lowercase hex HMAC-SHA1. Used by RFC 6238 TOTP and a few legacy signing schemes; prefer `hmac_sha256_hex` for new signing code.
