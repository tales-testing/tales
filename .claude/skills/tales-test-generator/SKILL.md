...
- `website/src/content/docs/docs/providers/file.mdx` (file provider reference)
- `website/src/content/docs/docs/providers/exec.mdx` (exec provider reference)
- `website/src/content/docs/docs/providers/rpc.mdx` (rpc provider reference: ConnectRPC + gRPC, descriptors, reflection)
- `internal/runtime/rpc.go` (rpc execution: target/service/method evaluation + provider dispatch + assertions)
...
- HTTP `save { body = "<path>" }` writes the raw response body under `scenario.workdir` and exposes `response.download.{path,size_bytes,sha1..sha512_256}`. Paths escaping the workspace are rejected. `response.body` stays a string.
...
- Enterprise identifier generators (`siren`, `siret`, `ein`, `duns`, `lei`) take no options and return strings: `siren` is a 9-digit French SIREN (valid Luhn), `siret` is a 14-digit French SIRET (valid Luhn, first 9 digits are a Luhn-valid SIREN), `ein` is a US `NN-NNNNNNN` Employer Identification Number, `duns` is a 9-digit Dun & Bradstreet number, `lei` is a 20-character ISO 17442 Legal Entity Identifier with the ISO 7064 mod 97-10 check.
- `vat_number` and `euid` accept an optional `country` (ISO 3166-1 alpha-2 EU member code among `AT`, `BE`, `BG`, `CY`, `CZ`, `DE`, `DK`, `EE`, `EL`, `ES`, `FI`, `FR`, `HR`, `HU`, `IE`, `IT`, `LT`, `LU`, `LV`, `MT`, `NL`, `PL`, `PT`, `RO`, `SE`, `SI`, `SK`); empty or unknown values fall back to a random EU member. `vat_number` returns an EU VAT registration number with the per-country algorithm (Luhn for IT, mod 97 for FR/BE, mod 11 for PT/SK, format-only elsewhere). `euid` returns a BRIS-style `CC.RegisterID.EntityID.Check` per Regulation (EU) 2015/884.
- `expect.body` can be used for raw response-body assertions, including matchers such as `contains(...)`.
- HTTP response shape: `response.status` (number), `response.json` (decoded JSON or null), `response.body` (raw string), `response.headers` (map of `list(string)`, every value preserved in wire order and keyed by the canonical MIME name; single-valued headers are always one-element lists, so write `response.headers["Content-Type"][0]` for the common case and `response.headers["Set-Cookie"]` for the full list), `response.cookies` (map of parsed cookie objects with `name`, `value`, `raw`, `path`, `domain`, `expires`, `max_age`, `secure`, `http_only`, `same_site`; duplicate cookie names use the last-seen value). Cookie names that are not HCL identifiers (e.g. `ia-session`) require index syntax: `response.cookies["ia-session"].value`. With a `save { body = "..." }` block, `response.download` adds `{path, size_bytes, sha1..sha512_256}` (only then).
- Workspace variables (any expression): `scenario.workdir` (writable per-scenario dir), `scenario.artifacts_dir`, `scenario.name`, and `project.dir` (project root). Relative `save`/`file`/`exec` paths resolve under `scenario.workdir`; project files via `${project.dir}/...`.
- `retry.attempts` must be `>= 1`.
- `retry.interval` must be a valid Go duration string such as `"100ms"`, `"500ms"`, or `"2s"`.
- Matchers/functions available: `contains`, `matches`, `exists`, `not_exists`, `is_string`, `is_number`, `is_bool`, `is_array`, `is_object`, `one_of`, `uuid`, `can`, `regex_find`, `url_encode`, `jsonencode`, `now_unix`, `now_rfc3339`, `sha1_hex`, `sha224_hex`, `sha256_hex`, `sha384_hex`, `sha512_hex`, `sha512_224_hex`, `sha512_256_hex`, `hmac_sha1_hex`, `hmac_sha224_hex`, `hmac_sha256_hex`, `hmac_sha384_hex`, `hmac_sha512_hex`, `hmac_sha512_224_hex`, `hmac_sha512_256_hex`, `base64url_encode`, `pkce_challenge`, `totp`, `optional`, `required`, `any`.
...
- `scroll_to { id = "..." }` (or `label = "..."` / `text = "..."`) —
  ...
  failure is a locator that matches no element. Takes no `timeout` /
  `interval`. Pair it with `input_text` when typing into a field that
  ...
  `scroll_to` is the safe default when generating scenarios that walk
  through a form longer than the viewport. Because it is best-effort,
  assert with a following `expect { visible }` when the scroll itself
